...
<div class="panel panel-default">
        ...
        <ul class="list-group">
            ...
            <li class="list-group-item">
                ...
                <div class = "listIM" v-for = "(item,index) in size" :class="{active: flag3 == index}" :key = "index" @click="activeFun3(index)">
                    <a>{{ item.name }}</a>
                </div>
            </li>
            <li class="list-group-item">
                <div class = "title">&nbsp;&nbsp;&nbsp;分辨率：</div>
                <div class = "listIM" v-for = "(item,index) in resolution" :class="{active: flag4 == index}" :key = "index" @click="activeFun4(index)">
                         <a>{{ item.name }}</a>
                </div>
            </li>
            <li class="list-group-item">
                <div class = "title">屏幕比例：</div>
                <div class = "listIM" v-for = "(item,index) in proportion" :class="{active: flag5 == index}" :key = "index" @click="activeFun5(index)">
                    <a>{{ item.name }}</a>
                </div>
            </li>
        </ul>
